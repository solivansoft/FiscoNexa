param(
  [string]$Executable = "$(Join-Path $PSScriptRoot '..\bin\tests\win64\FiscoNexa.MonitorLeasesIntegration.exe')"
)
...
& cmd /c (Join-Path $root 'scripts\test-monitor-leases.bat')
if ($LASTEXITCODE -ne 0) { throw 'Falha ao compilar smoke de lease.' }

& $Executable
if ($LASTEXITCODE -ne 0) { throw 'Smoke de lease falhou.' }
